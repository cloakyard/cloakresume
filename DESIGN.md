...
Animate inner content only; never animate this geometry.
...
at a `0.6rem` gap. The footer alone uses 34px. Never redraw, crop, stretch, or
independently resize the wordmark.
...
- The Ft5 footer is a dark Night statement band with the 34px family mark.
...
- Standard dialogs use 36rem. Template galleries and ATS review alone may use the
  68.75rem wide variant. Desktop dialogs are centred and stay within
  `calc(100dvh - 3rem)`.
...
- Fast feedback is 160ms, touch/sheet movement 220ms, and slow UI 260ms. The 400ms
  token is reserved for one-time structural reveals.
- Use `cubic-bezier(0.16, 1, 0.3, 1)`. Active controls move at most 1px and never
  overshoot. Never use `transition: all`.
...
- Reduced-motion removes nonessential transforms/smooth scrolling while preserving
  immediate state and progress communication.
