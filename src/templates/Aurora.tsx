...
export const Aurora = memo(function Aurora({ resume, palette }: Props) {
  ...
  const css = `
    .au-root { font-family: 'Geist Variable', 'Inter', sans-serif; color: #1f2937; font-size: 9.4pt; line-height: 1.5; overflow-wrap: break-word; word-break: normal; hyphens: manual; }
    ...
    .au-jobdates { font-size: 8.2pt; color: ${palette.primary700}; font-weight: 700; white-space: normal; font-variant-numeric: tabular-nums; flex-shrink: 0; max-width: 100%; min-width: 0; }
    ...
    .au-skill-label > :not(svg) { min-width: 0; overflow-wrap: anywhere; }
    ...
    .au-edu-meta { font-size: 8.4pt; color: #6b7280; font-variant-numeric: tabular-nums; text-align: right; flex-shrink: 0; max-width: 100%; min-width: 0; }
      ...
      {resume.quickStats.length > 0 && (
        <div className="au-stats">
          {resume.quickStats.map((s) => (
            <div className="au-stat" data-stat-card="true" key={s.id}>
          ...
          {pair.map((g) => {
            ...
            return (
              <div className="au-skill" key={g.id}>
                <div className="au-skill-label" data-keep-with-next="true">
    ...
    resume.projects.forEach((p) => {
      atoms.push(
        <div className="au-proj-card" key={`proj-${p.id}`}>
          <div className="au-proj-name" data-keep-with-next="true">
            {p.name}
          </div>
          {p.description && (
            <>
              <div className="au-proj-label" data-keep-with-next="true">
                About Project
              </div>
