...
export const AtsPlain = memo(function AtsPlain({ resume }: Props) {
  const css = `
    .ats-root { font-family: Arial, Helvetica, 'Liberation Sans', sans-serif; color: #000; font-size: 10.5pt; line-height: 1.45; overflow-wrap: break-word; word-break: normal; hyphens: manual; }
    ...
    resume.projects.forEach((p) => {
      atoms.push(
        <div className="ats-proj" key={`proj-${p.id}`}>
          ...
          {p.description && (
            <>
              <div className="ats-proj-label" data-keep-with-next="true">
                About Project
              </div>
