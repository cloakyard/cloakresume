...
export const AtsProfessional = memo(function AtsProfessional({ resume, palette }: Props) {
  const css = `
    .atp-root { font-family: 'Geist Variable', 'Inter', 'Calibri', Arial, Helvetica, sans-serif; color: #111827; font-size: 10.5pt; line-height: 1.5; overflow-wrap: break-word; word-break: normal; hyphens: manual; }
    ...
    .atp-location { font-size: 9.8pt; color: #6b7280; overflow-wrap: break-word; max-width: 100%; min-width: 0; }
    ...
    resume.projects.forEach((p) => {
      atoms.push(
        <div className="atp-entry" key={`proj-${p.id}`}>
          ...
          {p.description && (
            <>
              <div className="atp-proj-label" data-keep-with-next="true">
                About Project
              </div>
