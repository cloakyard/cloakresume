...
export const ClassicSidebar = memo(function ClassicSidebar({ resume, palette }: Props) {
  ...
  const css = `
    .cs-root { font-family: 'Geist Variable', 'Inter', sans-serif; color: #1f2937; font-size: 9.2pt; line-height: 1.4; overflow-wrap: break-word; word-break: normal; hyphens: manual; }
    ...
    .cs-skill-label > :not(svg) { min-width: 0; overflow-wrap: anywhere; }
    ...
    .cs-summary { font-size: 9pt; line-height: 1.55; color: #1e293b; text-align: justify; hyphens: manual; overflow-wrap: break-word; }
    ...
    .cs-edumeta { font-size: 8.4pt; color: #6b7280; font-style: italic; flex-shrink: 0; text-align: right; max-width: 100%; min-width: 0; }
      ...
      sidebarAtoms.push(
        <div className="cs-sb-atom" key={`skill-${group.id}`}>
          ...
          <div className="cs-skill-group">
            <div className="cs-skill-label" data-keep-with-next="true">
    ...
    resume.projects.forEach((p) => {
      mainAtoms.push(
        <div className="cs-proj" key={`proj-${p.id}`}>
          <div className="cs-projname" data-keep-with-next="true">
            {p.name}
          </div>
          {p.description && (
            <>
              <div className="cs-proj-label" data-keep-with-next="true">
                About Project
              </div>
