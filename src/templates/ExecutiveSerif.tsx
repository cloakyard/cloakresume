...
export const ExecutiveSerif = memo(function ExecutiveSerif({ resume, palette }: Props) {
  ...
  const css = `
    .es-root { font-family: 'Geist Variable', 'Inter', sans-serif; color: #1f2937; font-size: 9.4pt; line-height: 1.5; overflow-wrap: break-word; word-break: normal; hyphens: manual; }
    ...
    .es-summary { font-size: 9.5pt; line-height: 1.65; text-align: justify; color: #1e293b; hyphens: manual; overflow-wrap: break-word; }
    ...
    .es-skill-label > :not(svg) { min-width: 0; overflow-wrap: anywhere; }
    ...
    .es-edumeta { color: #6b7280; font-size: 8.8pt; font-style: italic; flex-shrink: 0; max-width: 100%; min-width: 0; }
    ...
    resume.skills.forEach((g) => {
      ...
      atoms.push(
        <div key={`skills-${g.id}`} className="es-skill-group">
          <div className="es-skill-label" data-keep-with-next="true">
    ...
    resume.projects.forEach((p) => {
      atoms.push(
        <div className="es-proj" key={`proj-${p.id}`}>
          <div className="es-projname" data-keep-with-next="true">
            {p.name}
          </div>
          {p.description && (
            <>
              <div className="es-proj-label" data-keep-with-next="true">
                About Project
              </div>
