...
export const Horizon = memo(function Horizon({ resume, palette }: Props) {
  ...
  const css = `
    .hz-root { font-family: 'Geist Variable', 'Inter', sans-serif; color: #111827; font-size: 9.3pt; line-height: 1.48; overflow-wrap: break-word; word-break: normal; hyphens: manual; }
    ...
    .hz-skill-label > :not(svg) { min-width: 0; overflow-wrap: anywhere; }
    ...
    .hz-jobdates { font-size: 8.4pt; color: ${palette.primary700}; font-weight: 700; font-variant-numeric: tabular-nums; flex-shrink: 0; max-width: 100%; min-width: 0; }
    ...
    .hz-edumeta { font-size: 8.4pt; color: #6b7280; font-variant-numeric: tabular-nums; text-align: right; flex-shrink: 0; max-width: 100%; min-width: 0; }
      ...
      sidebarAtoms.push(
        <div className="hz-sb-atom" key={`skill-${group.id}`}>
          ...
          <div className="hz-skill-group">
            <div className="hz-skill-label" data-keep-with-next="true">
    ...
    resume.projects.forEach((p) => {
      mainAtoms.push(
        <div className="hz-proj" key={`proj-${p.id}`}>
          <div className="hz-projname" data-keep-with-next="true">
            {p.name}
          </div>
          {p.description && (
            <>
              <div className="hz-proj-label" data-keep-with-next="true">
                About Project
              </div>
