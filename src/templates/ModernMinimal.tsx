...
export const ModernMinimal = memo(function ModernMinimal({ resume, palette }: Props) {
  ...
  const css = `
    .mm-root { font-family: 'Geist Variable', 'Inter', sans-serif; color: #1f2937; font-size: 9.5pt; line-height: 1.45; overflow-wrap: break-word; word-break: normal; hyphens: manual; }
    ...
    .mm-skill-label > :not(svg) { min-width: 0; overflow-wrap: anywhere; }
    ...
    .mm-edumeta { color: #6b7280; font-size: 8.6pt; flex-shrink: 0; max-width: 100%; min-width: 0; }
    ...
    resume.skills.forEach((g) => {
      ...
      atoms.push(
        <div key={`skills-${g.id}`} className="mm-skill-col">
          <div className="mm-skill-label" data-keep-with-next="true">
    ...
    resume.projects.forEach((p) => {
      atoms.push(
        <div className="mm-proj" key={`proj-${p.id}`}>
          <div className="mm-projhead" data-keep-with-next="true">
            <div className="mm-projname" data-keep-with-next="true">
              {p.name}
            </div>
          </div>
          {p.description && (
            <>
              <div className="mm-proj-label" data-keep-with-next="true">
                About Project
              </div>
