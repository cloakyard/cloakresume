...
export const Monograph = memo(function Monograph({ resume, palette }: Props) {
  ...
  const css = `
    .mg-root { font-family: 'Geist Variable', 'Inter', sans-serif; color: #1c1917; font-size: 9.3pt; line-height: 1.5; overflow-wrap: break-word; word-break: normal; hyphens: manual; }
    ...
    .mg-skill-label > :not(svg) { min-width: 0; overflow-wrap: anywhere; }
    ...
    .mg-summary { font-size: 9.5pt; line-height: 1.65; color: #292524; text-align: justify; hyphens: manual; overflow-wrap: break-word; }
    ...
    .mg-edumeta { font-size: 8.4pt; color: #78716c; font-style: italic; font-variant-numeric: tabular-nums; text-align: right; flex-shrink: 0; max-width: 100%; min-width: 0; }
      ...
      sidebarAtoms.push(
        <div className="mg-sb-atom" key={`skill-${group.id}`}>
          ...
          <div className="mg-skill-group">
            <div className="mg-skill-label" data-keep-with-next="true">
    ...
    resume.projects.forEach((p) => {
      mainAtoms.push(
        <div className="mg-proj" key={`proj-${p.id}`}>
          <div className="mg-projname" data-keep-with-next="true">
            {p.name}
          </div>
          {p.description && (
            <>
              <div className="mg-proj-label" data-keep-with-next="true">
                About Project
              </div>
