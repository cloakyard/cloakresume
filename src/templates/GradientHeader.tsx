...
export const GradientHeader = memo(function GradientHeader({ resume, palette }: Props) {
  ...
  const css = `
    .gh-root { font-family: 'Geist Variable', 'Inter', sans-serif; color: #1f2937; font-size: 9.4pt; line-height: 1.5; overflow-wrap: break-word; word-break: normal; hyphens: manual; }
    ...
    .gh-skill-label > :not(svg) { min-width: 0; overflow-wrap: anywhere; }
    ...
    .gh-edumeta { color: #6b7280; font-size: 8.4pt; font-style: italic; flex-shrink: 0; max-width: 100%; min-width: 0; }
    ...
    resume.skills.forEach((g) => {
      ...
      atoms.push(
        <div key={`skills-${g.id}`} className="gh-skill-row">
          <div className="gh-skill-label" data-keep-with-next="true">
    ...
    resume.projects.forEach((p) => {
      atoms.push(
        <div className="gh-proj" key={`proj-${p.id}`}>
          <div className="gh-projhead" data-keep-with-next="true">
            <div className="gh-projname" data-keep-with-next="true">
              {p.name}
            </div>
          </div>
          {p.description && (
            <>
              <div className="gh-proj-label" data-keep-with-next="true">
                About Project
              </div>
  ...
  if (resume.extras.length > 0) {
    atoms.push(
      <h2 className="gh-h2" data-keep-with-next="true" key="extras-h">
        Extras
      </h2>,
    );
