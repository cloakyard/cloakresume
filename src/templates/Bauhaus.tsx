...
export const Bauhaus = memo(function Bauhaus({ resume, palette }: Props) {
  ...
  const css = `
    .bh-root { font-family: 'Geist Variable', 'Inter', sans-serif; color: #111827; font-size: 9.4pt; line-height: 1.5; overflow-wrap: break-word; word-break: normal; hyphens: manual; }
    .bh-head { display: grid; grid-template-columns: 42mm minmax(0, 1fr) 60mm; grid-template-rows: minmax(18mm, auto) minmax(22mm, auto); gap: 0; margin: 0 0 6mm; border: 2px solid #0a0a0a; }
    .bh-logo { position: relative; background: ${palette.primary600}; color: ${palette.primaryText}; display: flex; align-items: center; justify-content: center; grid-row: 1 / span 2; border-right: 2px solid #0a0a0a; font-size: 22pt; font-weight: 900; letter-spacing: -1px; overflow: hidden; min-width: 0; }
    .bh-logo img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
    .bh-name-block { background: #0a0a0a; color: #ffffff; display: flex; align-items: center; padding: 6mm; grid-column: 2; grid-row: 1 / span 2; border-right: 2px solid #0a0a0a; min-width: 0; }
    ...
    .bh-jobdates { font-size: 8.4pt; font-weight: 700; color: #ffffff; background: #0a0a0a; padding: 0.6mm 2.2mm; font-variant-numeric: tabular-nums; white-space: normal; letter-spacing: 0.4px; flex-shrink: 0; max-width: 100%; min-width: 0; }
    ...
    .bh-skill-label > :not(svg) { min-width: 0; overflow-wrap: anywhere; }
    ...
    .bh-edu-meta { font-size: 8.4pt; color: #0a0a0a; font-variant-numeric: tabular-nums; white-space: normal; font-weight: 700; max-width: 100%; min-width: 0; }
    ...
    atoms.push(
      <div className="bh-stats" key="stats">
        {resume.quickStats.map((s) => (
          <div className="bh-stat" data-stat-card="true" key={s.id}>
          ...
          {resume.skills.map((g) => {
            ...
            return (
              <div className="bh-skill" key={g.id}>
                <div className="bh-skill-label" data-keep-with-next="true">
      ...
      atoms.push(
        <div className="bh-proj" key={`proj-${p.id}`}>
          ...
          <div className="bh-proj-body">
            <div className="bh-proj-name" data-keep-with-next="true">
              {p.name}
            </div>
            {p.description && (
              <>
                <div className="bh-proj-label" data-keep-with-next="true">
                  About Project
                </div>
