...
export const Minimalist = memo(function Minimalist({ resume, palette }: Props) {
  const css = `
    .ml-root { font-family: 'Geist Variable', 'Inter', sans-serif; color: #1a1a1a; font-size: 9.8pt; line-height: 1.6; overflow-wrap: break-word; word-break: normal; hyphens: manual; }
    ...
    .ml-summary { font-size: 10pt; line-height: 1.7; color: #262626; max-width: 168mm; hyphens: manual; overflow-wrap: break-word; }
    ...
    .ml-skill-label > :not(svg) { min-width: 0; overflow-wrap: anywhere; }
    ...
    .ml-edu-meta, .ml-proj-meta { font-size: 9pt; color: #737373; font-variant-numeric: tabular-nums; flex-shrink: 0; overflow-wrap: break-word; max-width: 100%; min-width: 0; }
    ...
    resume.skills.forEach((g) => {
      atoms.push(
        <div className="ml-skill" key={`skills-${g.id}`}>
          <div className="ml-skill-label" data-keep-with-next="true">
            {g.label}
          </div>
    ...
    resume.projects.forEach((p) => {
      atoms.push(
        <div className="ml-proj" key={`proj-${p.id}`}>
          <div className="ml-proj-head" data-keep-with-next="true">
            <div className="ml-proj-name" data-keep-with-next="true">
              {p.name}
            </div>
          </div>
          {p.description && (
            <>
              <div className="ml-proj-label" data-keep-with-next="true">
                About Project
              </div>
