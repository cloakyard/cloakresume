...
**Initial audit:** 22 July 2026 · **Inventory and geometry updated:** 14 September 2026

See [the September feature audit](feature-audit-2026-09-14.md) and [release verification](release-audit-2026-09-14.md) for the current results. The July verification below remains a historical record.
...
All 73 TSX files in `src/` were examined. No application route, common component,
...
The 17 résumé templates are deliberately exempt from product-chrome styling. They are
...
| Geometry            | Headers, panels, repeated entries, popovers, and sheets carried local width and edge values.                    | Shared 72/64px headers, 72px rail, 328px panel and fluid 384–736px wide-screen panel, exact 40px desktop toolbar controls, exact 44px mobile segments, complete 1px/12px entry cards, 40px mark, 16px popover edge, and common overlay widths are authoritative. |
...
| Motion              | Broad transitions, animated focus shadows, direct SVG transforms, and one-way overlays could destabilize UI.    | Tokenized press/reveal/landing motion uses compositor-safe wrappers, synchronized 220ms overlay entrances, 160ms exits, immediate focus rings, stable editor/dialog geometry, and a reduced-motion bypass.                                                       |
...
| Loading cost        | Template and review work could be pulled into the initial route or repeated unnecessarily.                      | ATS review, PDF export, Harper, and all 17 résumé templates use explicit lazy boundaries; landing does not fetch template implementations, and gallery work begins only on visible user intent.                                                                  |
...
|   5 | `src/components/CloakWorkbenchLanding.tsx`        | Aligned | N1b header, 88rem frame, real start/load instrument, ledgers, local receipt, and CloakPDF-parity Ft5 footer with version/author provenance.                                       |
...
|  13 | `src/components/Layout.tsx`                       | Aligned | 64px task-only editor header, 72px rail, token-driven 328px panel with a fluid 384–736px wide-screen range, 50:50 mobile proof/workspace split, inline picker, and one editor h1. |
...
|  20 | `src/components/PrivacyPolicyModal.tsx`           | Aligned | Wide architecture receipt, ruled long-form policy, accurate local/network boundary, bottom-sheet collapse, focus lifecycle, and overscroll containment.                           |
...
|  23 | `src/components/SectionPanel.tsx`                 | Aligned | Responsive 328px desktop editor and fluid 384–736px wide-screen editor, 50% phone pane, close-to-picker action, title hierarchy, internal scroll, and safe area.                  |
...
|  28 | `src/components/ToolbarCenter.tsx`                | Aligned | Named colour popover, placement, 16px safe edge, animated presence/focus return, and document-colour boundary.                                                                    |
|  29 | `src/components/ToolbarOverflow.tsx`              | Aligned | Task-only actions, exact 44px tablet/mobile trigger, sequenced overlay handoff, solid surface, and predictable dismissal.                                                         |
...
|  50 | `src/components/editor/shared.tsx`                | Aligned | Complete shared entry cards, narrow-screen header fit, delete/add/reorder controls, labels, 44px targets, and rule/radius tokens.                                                 |
...
|  58 | `src/templates/ClassicImpact.tsx`                 | Output  | Classic sidebar with summary-adjacent results, row-level stats pagination, semantic headings, and per-item supporting facts.                                                      |
|  59 | `src/templates/ClassicSidebar.tsx`                | Output  | Main/sidebar ownership, sidebar pagination, complete sections, and continuation context.                                                                                          |
|  60 | `src/templates/CompactTimeline.tsx`               | Output  | Dense hierarchy, timeline continuity, bullet splitting, and one-/multi-page behavior.                                                                                             |
|  61 | `src/templates/ExecutiveSerif.tsx`                | Output  | Serif hierarchy, complete sections, safe contrast, and pagination boundaries.                                                                                                     |
|  62 | `src/templates/GradientHeader.tsx`                | Output  | Document-only gradient exception, text contrast, complete sections, and print output.                                                                                             |
|  63 | `src/templates/Horizon.tsx`                       | Output  | Timeline/sidebar structure, section coverage, continuation behavior, and page flow.                                                                                               |
|  64 | `src/templates/Ledger.tsx`                        | Output  | Single-column professional brief, ruled stats ledger, complete data coverage, and fine-grained pagination atoms.                                                                  |
|  65 | `src/templates/Minimalist.tsx`                    | Output  | Typographic hierarchy, visible headings, complete sections, and restrained pagination.                                                                                            |
|  66 | `src/templates/ModernMinimal.tsx`                 | Output  | Grid reading order, complete sections, bullet atoms, and A4/Letter behavior.                                                                                                      |
|  67 | `src/templates/Monograph.tsx`                     | Output  | Editorial/sidebar hierarchy, complete sections, sidebar atoms, and continuation behavior.                                                                                         |
|  68 | `src/templates/Prism.tsx`                         | Output  | Accent sidebar contrast, complete sections, sidebar pagination, and output fidelity.                                                                                              |
|  69 | `src/templates/Typographic.tsx`                   | Output  | Numbered hierarchy, semantic equivalents, complete sections, and page flow.                                                                                                       |
|  70 | `src/templates/paginationAtoms.tsx`               | Output  | Shared head/bullet atoms, keep-with-next, stable keys, and safe split boundaries.                                                                                                 |
|  71 | `src/templates/shared.tsx`                        | Output  | Shared contact/date/location rendering, safe links, semantics, and consistent text output.                                                                                        |
|  72 | `src/utils/fieldIssues.tsx`                       | Support | Issue context, focus/highlight target, accessible descriptions, and stable hint layout.                                                                                           |
|  73 | `src/utils/richText.tsx`                          | Output  | Sanitized selectable rich text, safe links, semantic formatting, and PDF text extraction.                                                                                         |
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Landing and responsive shell  | The landing passed at 320, 375, 414, 768, 834, 1024, 1280, 1440, and 1600px without horizontal overflow. Its three-block entry retained 0/60/120ms staging without moving layout geometry; the header remained 72px and its mark 40px.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Editor geometry               | Every one of the 13 editor sections was opened at 320, 768, and 1280px. All nine repeatable-entry implementations rendered a complete 1px border, 8px radius, 12px inset, and no panel overflow; the longest `Certification #1` header also remained contained at 320px. Mobile and tablet retained exact 368/368px and 480/480px proof/workspace splits, 44px header controls, one open section or the picker, and zero geometry transition. Desktop retained the 72px rail and 328/384px panel contract. The editor header no longer exposes provenance copy or a GitHub action; those remain on public/info surfaces. Every visible desktop toolbar component, including the A4/Letter fieldset, remained exactly 40px high. |
| Overlays and popovers         | Options/template/colour flows were exercised as full-width sheets at 320/414/639px and centred overlays at 640/768/834/1024/1280/1440px with no clipping or horizontal overflow. Scrim and panel now enter together at 220ms; overlay and popover exits stay mounted through their 160ms motion, block pointer input during dismissal, and restore trigger focus. Chained mobile overlays wait for the previous sheet to exit, preventing stacked focus traps. The 288px month/year popover retains its 16px safe edge and every fixed popover preserves its positioning transform by using opacity-only presence motion.                                                                                                       |
